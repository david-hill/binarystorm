...
function configure_firewall {
  ports="25/tcp 53/tcp 53/udp 80/tcp 110/tcp 143/tcp 143/udp 161/udp 443/tcp 587/tcp 993/tcp 995/tcp 5000/tcp"
  reload=0
  for p in $ports; do
    add_port $p
    reload $?
...
function install_packages_and_update {
  ...
  packages="uptimed vim yum-utils telnet wget ntp tmux sysstat gcc tcp_wrappers whois podman createrepo_c"
  for p in $packages; do 
    install_package $p
  done
  yum update -y
