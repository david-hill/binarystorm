...
if [ $? -eq 0 ]; then
  ...
  podman rm freshclam
  podman pull $registry/freshclam-root:latest
fi
...
chown 995 /var/lib/clamav
podman run -d --network ipv6 --ip6 fd00::10 --ip 10.89.0.10 -h $(hostname) -v /root/binarystorm/etc/freshclam.conf:/etc/freshclam.conf:ro -v /var/lib/clamav:/var/lib/clamav --mount=type=bind,src=/dev/log,dst=/dev/log --hosts-file ../common/hosts --name=freshclam $registry/freshclam-root:latest
sleep 3
