...
podman  run -d --network ipv6 --ip6 fd00::8 --ip 10.89.0.8 -h $(hostname) -v /root/binarystorm/etc/clamd.d:/etc/clamd.d:ro -v /var/run/clamd.scan/:/var/run/clamd.scan/ -v /var/spool/amavisd/tmp/:/var/spool/amavisd/tmp/ -v /var/lib/clamav:/var/lib/clamav:ro -v /run/clamd.amavisd:/run/clamd.amavisd --mount=type=bind,src=/dev/log,dst=/dev/log --hosts-file ../common/hosts --name=clamd $registry/clamd-root:latest
sleep 3
